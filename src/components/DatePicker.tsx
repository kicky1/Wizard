...
import { Control, useController, FieldPath, FieldValues, PathValue } from "react-hook-form";
import {
  Button,
  CalendarCell,
  CalendarGrid,
...
> {
  ...
  startDateName: TName;
  endDateName: TName;
  defaultStartDate?: PathValue<TFieldValues, TName>;
  defaultEndDate?: PathValue<TFieldValues, TName>;
  label?: string;
}
...
>({ 
  ...
  const dateRangeValue = startDateField.value && endDateField.value ? {
    start: parseDate(startDateField.value as string),
    end: parseDate(endDateField.value as string)
  } : undefined;
  ...
  return (
    <DateRangePicker 
      ...
      onChange={(range) => {
        if (range) {
          startDateField.onChange(range.start.toString());
          endDateField.onChange(range.end.toString());
        }
      }}
      aria-label={label}
    >
